...
function MHeader({ navitems, title, close, open, anchor, favLength = 0, cartCount = 0 }) {
  const theme = useTheme();
  const classes = useStyles();
  return (
    <AppBar elevation={0} position="fixed">
      <Toolbar className={classes.toolbar}>
        <Button aria-controls="simple-menu" aria-haspopup="true" onClick={(event) => open(event)}>
          <ProfileIcon />
        </Button>
        <Menu
          id="long-menu"
        ...
        >
          {navitems.map((item, index) => (
            <Box key={index}>
              <MenuItem className={classes.menuItem} onClick={() => close(item.title)}>
        ...
        <Box display="flex">
          <Box mr={theme.spacing(3)}>
            <RouterLink to="/favourite">{favLength > 0 ? <HeartActiveIcon /> : <HeartIcon />}</RouterLink>
          </Box>
          {cartCount && (
